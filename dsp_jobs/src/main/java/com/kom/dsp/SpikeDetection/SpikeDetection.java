...
public class SpikeDetection {
    ...
    DataStream<AverageValue> spikeDetector =
        averageCalculator
            .filter(new FilterFunction<AverageValue>() {
                @Override
                public boolean filter(AverageValue value) {
                    float relativeDifference = Math.abs(value.getCurrentValue() - value.getAverageValue()) / value.getAverageValue();
